...
p.vector.1 <- c(meanlog=-2,sdlog=2,t0=1) 
p.vector.2 <- c(meanlog=1,sdlog=2,t0=1)
...
a<-fitdistr(data.test[(as.integer(factor(data.test[,2]))==1),1],dens=dSLOGNO,lower=c(-Inf,0.1,0),upper=c(Inf,Inf,max(state.1)),start=list(meanlog=0,sdlog=1,shift=0),method="L-BFGS-B",control=list(maxit=500,pgtol=1e-20,trace=TRUE))
b<-fitdistr(data.test[(as.integer(factor(data.test[,2]))==2),1],dens=dSLOGNO,lower=c(-Inf,0.1,0),upper=c(Inf,Inf,min(state.2)),start=list(meanlog=0,sdlog=1,shift=0),method="L-BFGS-B",control=list(maxit=500,pgtol=1e-20,trace=TRUE))
...
# limits
lb <- c(-10,0,0)
ub <- c(10,10,max(state.1))

#controls
max.its <- 1000
nparticles <- 100
#ps optim - lower bound includes zero
res<-psoptim(c(0,0,0),obj,lower=lb,upper=ub,w=rep(1,length(RT)),rt=RT,control=list(trace=T,abstol=1e-2,reltol=1e-8,maxit=1e3))
        
#DEoptim - lower bound excludes zero
res <-DEoptim(obj, lower=lb, upper=ub, control=DEoptim.control(
  itermax=max.its, trace=T, NP=100), rt=RT,w=rep(1,length(RT)))
...
rModels2 <- list(
  list(
    s_lnorm_fitdist(RT,pstart=c(-3,3,1))
  ),
  list(
    s_lnorm_fitdist(RT,pstart=c(4,2.5,1))
  )
)
...
transition[[2]] <- transInit(~1,nstates=2,data=data.test,pstart=trstart[3:4])#,family=multinomial("identity"))

instart=runif(2)
inMod <- transInit(~1,ns=2,ps=instart, data=data.frame(rep(1,1)),family=multinomial("identity"))

mod2 <- makeDepmix(response=rModels2,transition=transition,prior=inMod,ntimes=N, 
                   homogeneous=FALSE)
fm3 <- fit(mod2,emc=em.control(rand=FALSE))

lb <- c(0,0,rep(0,4),-100,0,0,-100,0,0)
ub <- c(1,1,rep(1,4),100,100,100,100,100,100)
fm3.r <- fit(mod2,method='rsolnp',solnpcntrl=list(rho = 1, outer.iter = 500, inner.iter = 1000, 
                                            delta = 1e-10, tol = 1e-10),conrows=diag(12),
             conrows.lower=lb,
             conrows.upper=ub)

fm3.r <- fit(mod2,conrows=diag(12),
             conrows.lower=lb,
             conrows.upper=ub)

a<-solnp(c(-2,1,0),obj,y=state.1,w=1,LB=c(-10,0,0),UB=c(100,100,min(state.1)))
