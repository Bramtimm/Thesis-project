...
structure(list(family = c("SWald", "Shifted wald"),
               ...
               dldm = function(y,mu,sigma,nu){
                 dldm <- 1/mu - (mu*((y-nu)-gamma)^2)*(1/(gamma^2*(y-nu)))
                 dldm
               },
               #step2 done
               d2ldm2 = function(y,mu,sigma,nu){
                 d2ldm2 <- -1/mu^2 - ((y-nu)-sigma)^2*(1/(sigma^2*(y-nu)))
                 d2ldm2
               },
               #step3 done
               dldd = function(y,mu,sigma,nu){
                 dldd <- mu^2*((y-nu)-sigma)*(1/sigma^3)
                 dldd
               },
               #step4 done
               d2ldd2 = function(y,mu,sigma,nu){
                 d2ldd2 <- mu^2*(3*nu+2*sigma-3*y)*(1/sigma^4)
                 d2ldd2
               },
               #step5 done
               dldv = function(y,mu,sigma,nu){
                 dldv <- (nu^2*mu^2-2*nu*y*mu^2+y^2*mu^2-3*nu*sigma^2+3*y*sigma^2-mu^2*sigma^2)*(1/(2*(-nu+y)^2*sigma^2))
                 d1dv
               },
               #step6 done
               d2ldv2 = function(y,mu,nu){
                 d2ldv2 <- -(2*mu^2 + 3*nu-3*y)*(1/(2*(-nu+y))^3)
                 d2ldv2
               },
               #step7
               d2ldmdd = function(y,mu,sigma,nu){(2*mu*(y-nu)-sigma)*(1/sigma^3)},
               d2ldmdv = function(y,mu,sigma,nu){(mu*(-sigma+nu-y)*(sigma+nu-y))*(1/(sigma^2*(y-nu)^2))},
               d2ldddv = function(mu,sigma){-mu^2/sigma^3}, 
               ...
               nu.initial = expression({nu <- rep(max(y),length(y))}),
